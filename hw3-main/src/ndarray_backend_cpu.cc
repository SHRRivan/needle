 ...
 */
 /*
 * 这是一个实用程序结构，用于保持数组与 ALIGNMENT 边界对齐
 * 记忆。 此对齐方式应至少为 TILE * ELEM_SIZE，尽管我们将其调得更大
 * 这里使用默认值。
 */
struct AlignedArray {
  AlignedArray(const size_t size) {
    ...
    ptr = static_cast<scalar_t*>(_aligned_malloc(size * ELEM_SIZE, ALIGNMENT));
    if (!ptr) throw std::bad_alloc();
#else
    // Use posix_memalign for Unix/Linux
